  ...
  class extends HTMLElement {
    ...
    <div>
      ...
      <main>
        ...
        <p>
          <button class="button" id="open-proxy-passthrough" style="background-color: #6f42c1;">Open with Proxy (pass-through)</button>
        </p>
        <p>
          <button class="button" id="open-proxy-intercept" style="background-color: #e83e8c;">Open with Proxy (intercept)</button>
        </p>
        <p>
          <button class="button" id="open-proxy-test-server" style="background-color: #28a745;">Open Proxy Test Server</button>
        </p>
        <hr />
        <h2>System Bars</h2>
        <p>
    ...
    connectedCallback() {
      ...
      // Proxy test: pass-through (all requests go to network normally)
      self.shadowRoot
        .querySelector("#open-proxy-passthrough")
        .addEventListener("click", async function (e) {
          try {
            const handler = addProxyHandler(async (request) => {
              console.log("[Proxy] Intercepted:", request.method, request.url);
      ...
      // Proxy test: intercept (return custom response)
      self.shadowRoot
        .querySelector("#open-proxy-intercept")
        .addEventListener("click", async function (e) {
          try {
            const handler = addProxyHandler(async (request) => {
              console.log("[Proxy] Intercepted:", request.method, request.url);
              if (request.url.includes("example.com")) {
                return new Response(
                  "<html><body><h1>Proxied!</h1><p>This response was generated by the proxy handler.</p></body></html>",
                  {
                    status: 200,
      ...
      // Proxy test: open PHP test server with header injection
      self.shadowRoot
        .querySelector("#open-proxy-test-server")
        .addEventListener("click", async function (e) {
          try {
            let serverUrl = "http://localhost:8000/index.php";
            try {
            ...
            const handler = addProxyHandler(async (request) => {
              ...
              const headers = Object.assign({}, request.headers || {}, {
                "Capgo-Proxied-Request": "1",
              });
              try {
                // Use CapacitorHttp (native URLSession) to avoid CORS issues
                ...
                console.log("[Proxy] Response:", res.status, request.url);
                // CapacitorHttp with responseType:"blob" returns base64 string
                let base64Body = "";
                if (res.data != null) {
                  if (typeof res.data === "string") {
                    // responseType:"blob" returns raw base64 from native
                    base64Body = res.data;
                  } else {
                    // Fallback: object/JSON data — stringify and encode
                    const str = JSON.stringify(res.data);
                    base64Body = btoa(unescape(encodeURIComponent(str)));
                  }
                }
                // Return ProxyResponse directly (no fetch Response needed)
                return {
                  status: res.status,
                  headers: res.headers || {},
                  body: base64Body,
                ...
                console.error("[Proxy] CapacitorHttp error:", err);
                return null;
              }
            });
