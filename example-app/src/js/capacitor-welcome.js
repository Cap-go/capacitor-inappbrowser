...
import { SystemBars, SystemBarType, CapacitorHttp } from "@capacitor/core";
import {
  InAppBrowser,
  addProxyHandler,
  ToolBarType,
...
window.customElements.define(
  ...
  class extends HTMLElement {
    ...
    <div>
      ...
      <main>
        ...
        <p style="display: flex; gap: 8px; align-items: center;">
          <input type="text" id="custom-url-input" value="https://www.google.com" placeholder="https://example.com" style="flex: 1; padding: 8px; border: 1px solid #ccc; border-radius: 3px; margin-bottom: 10px; font-size: 0.9em; box-sizing: border-box;" />
        ...
        <p>
          <button class="button" id="open-proxy-google" style="background-color: #6f42c1;">Open Google with Proxy (pass-through)</button>
        </p>
        <p>
          <button class="button" id="open-proxy-google-header" style="background-color: #e83e8c;">Open Google with Proxy (intercept + header)</button>
        </p>
        <p>
          <button class="button" id="open-proxy-cal" style="background-color: #292929;">Open cal.com with Proxy (pass-through)</button>
        </p>
        <p>
          <button class="button" id="open-proxy-cal-firefox" style="background-color: #ff7139;">Open cal.com with Proxy (Firefox UA)</button>
        </p>
        <p>
          <button class="button" id="open-proxy-facebook" style="background-color: #1877F2;">Open Facebook with Proxy</button>
        </p>
        <hr />
        <h2>System Bars</h2>
        <p>
    ...
    connectedCallback() {
      ...
      // Proxy Google: pass-through (all requests go to network as-is)
      self.shadowRoot
        .querySelector("#open-proxy-google")
        .addEventListener("click", async function (e) {
          try {
            const handler = addProxyHandler(async (request) => {
              console.log("[Proxy Google] Pass-through:", request.method, request.url);
            ...
            console.error("Error opening proxy Google:", e);
          }
        });

      // Proxy Google: intercept and add custom header
      self.shadowRoot
        .querySelector("#open-proxy-google-header")
        .addEventListener("click", async function (e) {
          try {
            const handler = addProxyHandler(async (request) => {
              ...
              try {
                ...
                console.log("[Proxy Google] Response:", res.status, request.url);
                let base64Body = "";
                if (res.data != null) {
                ...
                console.error("[Proxy Google] CapacitorHttp error:", err);
                return null;
              }
            });
            ...
            console.error("Error opening proxy Google with header:", e);
          }
        });

      // Proxy cal.com: pass-through (all requests go to network as-is)
      self.shadowRoot
        .querySelector("#open-proxy-cal")
        .addEventListener("click", async function (e) {
          try {
            const handler = addProxyHandler(async (request) => {
              console.log("[Proxy cal.com] Pass-through:", request.method, request.url);
              return null; // pass-through
            });
            ...
            console.error("Error opening proxy cal.com:", e);
          }
        });

      // Proxy cal.com: intercept with Firefox macOS user agent via CapacitorHttp
      self.shadowRoot
        .querySelector("#open-proxy-cal-firefox")
        .addEventListener("click", async function (e) {
          const firefoxUA = "Mozilla/5.0 (Macintosh; Intel Mac OS X 15.7; rv:148.0) Gecko/20100101 Firefox/148.0";
          try {
            const handler = addProxyHandler(async (request) => {
              ...
              try {
                ...
                console.log("[Proxy cal.com Firefox] Response:", res.status, request.url);
                let base64Body = "";
                if (res.data != null) {
                  if (typeof res.data === "string") {
                    base64Body = res.data;
                ...
                console.error("[Proxy cal.com Firefox] CapacitorHttp error:", err);
                return null;
              }
            });
            ...
            console.error("Error opening proxy cal.com Firefox:", e);
          }
        });

      // Proxy Facebook: selective interception with CSP modification + executeScript
      self.shadowRoot
        .querySelector("#open-proxy-facebook")
        .addEventListener("click", async function (e) {
          const facebookUA = "Mozilla/5.0 (Macintosh; Intel Mac OS X 15.7; rv:148.0) Gecko/20100101 Firefox/148.0";
          try {
            const INJECT_SCRIPT = `console.log('[Facebook Inject] Script loaded successfully');`;

            const handler = addProxyHandler(async (request) => {
              const url = new URL(request.url);

              // Serve our injected script
              if (url.pathname === "/assets/facebook-inject.js") {
                console.log("[Proxy Facebook] Serving facebook-inject.js");
                return {
                  status: 200,
                  headers: { "Content-Type": "application/javascript" },
                  body: btoa(INJECT_SCRIPT),
                };
              }
              ...
              if (url.hostname.includes("facebook.com")) {
                ...
                try {
                  ...
                  console.log("[Proxy Facebook] Response:", res.status, request.url);
                  let base64Body = "";
                  if (res.data != null) {
                    if (typeof res.data === "string") {
                      base64Body = res.data;
                  ...
                  delete responseHeaders["x-frame-options"];
                  delete responseHeaders["Cross-Origin-Embedder-Policy-Report-Only"];
                  delete responseHeaders["cross-origin-embedder-policy-report-only"];
                  ...
                  console.error("[Proxy Facebook] CapacitorHttp error:", err);
                  return null;
                }
              } else {
                // Non-facebook URLs: pass-through
                console.log("[Proxy Facebook] Pass-through:", request.method, request.url);
                return null;
              }
            });
              ...
              InAppBrowser.addListener("browserPageLoaded", async () => {
                try {
                  await InAppBrowser.executeScript({
                    code: `
                      (function() {
                        console.log('[Facebook Proxy] Page loaded, injecting script');
                        var s = document.createElement('script');
                        s.type = 'module';
                        s.textContent = "import('/assets/facebook-inject.js').catch(e => console.log('[Facebook Proxy] Script import skipped:', e.message))";
                        document.head.appendChild(s);
                      })();
                    `,
                  });
                } catch (err) {
                  console.log("[Proxy Facebook] executeScript skipped:", err.message);
                }
              });
            ...
            console.error("Error opening proxy Facebook:", e);
          }
        });
