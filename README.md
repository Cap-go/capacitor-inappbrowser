...
| ---------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ----- |
| **`url`**                    | <code>string</code>  | Target URL to load. Remote `http://` and `https://` URLs are loaded as-is. Relative bundled paths such as `/index.html` are resolved to the Capacitor local URL before opening. Bundled asset serving itself requires `openWebView()`; system browser flows (`open()`) receive the resolved URL but cannot intercept packaged assets. |                    | 0.1.0 |
