...
while true; do
  ...
  if [ "$boot_completed" = "1" ] && [ "$package_ready" -eq 1 ] && [ "$settings_ready" -eq 1 ]; then
    exit 0
  fi
  ...
  if [ "$attempt" -ge "$max_attempts" ]; then
    ...
    echo "boot_completed=$boot_completed package_ready=$package_ready settings_ready=$settings_ready"
    exit 1
  fi
